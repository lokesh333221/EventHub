...
function OrganizerDashboardHome() {
  // Your original state variables
  const [totalOrganizerrevenue, setTotalOrganizerRevenue] = useState({})
  ...
  useEffect(() => {
    const getDashboardearnings = async () => {
      try {
        const response = await getOrganizerDashboardstats(auth?._id)
         console.log("response",response)
        if (response) {
          setTotalOrganizerRevenue(response)
        }
      } catch (error) {
  ...
  <CardContent>
    <div className="text-3xl font-bold text-white">
      {formatCurrency(totalOrganizerrevenue?.totalRevenue)}
    </div>
    {/* <div className="flex items-center mt-2">
      ...
      <span className="text-sm text-white/70 ml-2">from last month</span>
          ...
          <Card className="bg-gradient-to-br from-blue-500 to-blue-600 text-white shadow-lg hover:shadow-xl transition-all duration-300 transform hover:-translate-y-1">
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
              <CardTitle className="text-sm font-medium text-blue-100">Admin Revenue</CardTitle>
              <div className="h-12 w-12 bg-white/20 rounded-lg flex items-center justify-center">
                <IndianRupee className="h-6 w-6 text-white" />
              </div>
            </CardHeader>
            <CardContent>
              <div className="text-3xl font-bold text-white">{formatCurrency(totalOrganizerrevenue?.totalAmountFromOrganizers)}</div>
              {/* <div className="flex items-center mt-2">
                <ArrowUpRight className="h-4 w-4 text-white/80 mr-1" />
                <span className="text-sm text-white/90 font-medium">+14.2%</span>
                <span className="text-sm text-white/70 ml-2">from last month</span>
          ...
          <Card className="bg-gradient-to-br from-purple-500 to-pink-500 text-white shadow-lg hover:shadow-xl transition-all duration-300 transform hover:-translate-y-1">
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
              <CardTitle className="text-sm font-medium text-purple-100">Organizer Revenue</CardTitle>
             <div className="h-12 w-12 bg-white/20 rounded-lg flex items-center justify-center">
      <IndianRupee className="h-6 w-6 text-white" />
    </div>
            </CardHeader>
            <CardContent>
              <div className="text-3xl font-bold text-white">{formatCurrency(totalOrganizerrevenue?.totalOrganizerEarning)}</div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-1 gap-6">
          ...
          <Card className="bg-white shadow-lg">
            <MonthlyRevenueDashboard monthlyRevenue={totalOrganizerrevenue?.monthlyRevenue} />
          </Card>
